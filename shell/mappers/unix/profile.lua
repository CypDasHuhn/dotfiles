...
-- Add source line to an rc file if not already present
local function ensure_source_line(rc_path, profile_path, rc_name)
	...
	if not f then
		...
		f = io.open(rc_path, "w")
		if not f then return false, "Could not create " .. rc_name end
	...
	-- Append source line
	f = io.open(rc_path, "a")
	if not f then return false, "Could not write to " .. rc_name end
	f:write("\n# Dotfiles profile\n" .. source_line .. "\n")
	f:close()
...
-- Ensure both ~/.bashrc and ~/.zshrc source our profile.sh
function M.link(output_dir)
	...
	-- Link to both bash and zsh
	ensure_source_line(home .. "/.bashrc", profile, "~/.bashrc")
	ensure_source_line(home .. "/.zshrc", profile, "~/.zshrc")
...
function M.generate(vars, var_order, machine, modules_dir, output_dir)
	-- Get absolute paths
	local handle = io.popen('cd "' .. modules_dir .. '" && pwd')
	...
	for _, name in ipairs(var_order) do
		...
		if utils.should_include(var_def, machine.name, os_type, visual_type) then
...
%s() {
  cd "$%s"
	...
	-- Modules
	local platform_dir = abs_modules_dir .. "/unix"
