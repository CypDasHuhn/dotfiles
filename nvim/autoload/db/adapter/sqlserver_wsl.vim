function! db#adapter#sqlserver_wsl#canonicalize(url) abort
  let url = a:url
  if url =~# ';.*=' && url !~# '?'
...
function! s:sqlcmd_bin() abort
  return get(g:, 'db_sqlserver_sqlcmd', 'sqlcmd')
endfunction

function! s:is_windows_sqlcmd() abort
  let bin = s:sqlcmd_bin()
  return bin =~# '\c\.exe$'
endfunction

function! s:winpath(path) abort
  if !s:is_windows_sqlcmd()
    return a:path
  endif
...
function! db#adapter#sqlserver_wsl#interactive(url) abort
...
function! db#adapter#sqlserver_wsl#input(url, in) abort
  return db#adapter#sqlserver_wsl#interactive(a:url) + ['-i', s:winpath(a:in)]
endfunction

function! db#adapter#sqlserver_wsl#dbext(url) abort
  let url = db#url#parse(a:url)
  return {
