...
function M.detect_env()
    ...
    if env.os == "unix" then
        ...
        local distro_like = io.popen("cat /etc/os-release 2>/dev/null | grep '^ID_LIKE=' | cut -d= -f2 | tr -d '\"'"):read("*l")
                ...
                if distro_leaf then
                    ...
                    for arch_key, arch_val in pairs(distro_val) do
                        ...
                        if arch_leaf then
                            ...
                            result[os_key][distro_key][arch_key] = { command = tostring(arch_val), condition = nil, verify = nil }
                        end
                    end
                end
            end
...
function M.get_command(normalized, env)
    ...
    if not os_table then return nil end
    ...
    if not distro_table and env.distro_family then
        ...
        for family in env.distro_family:gmatch("%S+") do
            ...
            if distro_table then break end
        end
    end
    distro_table = distro_table or os_table["default"]
    if not distro_table then return nil end
